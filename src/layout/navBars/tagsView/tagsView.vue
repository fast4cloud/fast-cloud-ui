...
const isActive = (v: RouteItem) => {
	if (getThemeConfig.value.isShareTagsView) {
		...
		if ((v.query && Object.keys(v.query).length) || (v.params && Object.keys(v.params).length)) {
			...
			// https://gitee.com/lyt-top/ fast-cloud-ui/issues/I51RS9
			return v.path === state.routePath;
		}
	}
};
...
const addTagsView = (path: string, to?: RouteToFrom) => {
	// 防止拿取不到路由信息
	nextTick(async () => {
		// 修复：https://gitee.com/lyt-top/ fast-cloud-ui/issues/I3YX6G
		let item: RouteItem;
...
// https://gitee.com/lyt-top/ fast-cloud-ui/issues/I61VS9
...
// https://gitee.com/lyt-top/ fast-cloud-ui/issues/I5K3YO
// https://gitee.com/lyt-top/ fast-cloud-ui/issues/I61VS9
const transUrlParams = (v: RouteItem) => {
	...
	if (v.meta?.isDynamic) {
		 ...
		 * 2、通过事件调用时，路由的 path 不是原始定义的路由格式，如：/params/dynamic/details/ fast-cloud-ui/111/我是动态路由测试tagsViewName(非国际化)
		 *
...
// 拖动问题，https://gitee.com/lyt-top/ fast-cloud-ui/issues/I3ZRRI
